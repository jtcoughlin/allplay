...
class PosterDiagnosticChecker {
  ...
  private getSourceEmoji(source: string | null): string {
    switch (source) {
      case 'staticMap': return '📍'; // Pinned/Static mapping
      case 'tmdb': return '🎬'; // TMDb database
      case 'youtube': return '📺'; // YouTube thumbnail
      case 'fallback': return '🔄'; // Service fallback
      case 'missing': return '❓'; // Missing/unknown
      default: return '❓';
    }
  }
  ...
  async runDiagnostic(): Promise<void> {
    ...
    console.log('Checking specific problem titles for poster quality and storage...\n');
    ...
    for (const problemTitle of this.problemTitles) {
      ...
      const matchingItems = allContent.filter(item => 
        item.title && item.title.toLowerCase().includes(problemTitle.toLowerCase())
      );
      ...
      for (const item of matchingItems) {
        ...
        console.log(`      ${this.getUrlStatusEmoji(urlStatus)} Image URL: ${item.imageUrl || 'MISSING'}`);
        console.log(`      ${this.getSourceEmoji(result.posterSource)} Source: ${result.posterSource || 'UNKNOWN'}`);
        console.log(`      ${this.getLockedEmoji(result.posterLocked)} Locked: ${result.posterLocked ?? 'UNKNOWN'}`);
        
        if (urlStatus === 'broken') {
          console.log(`      ⚠️  URL VALIDATION FAILED - This poster is broken!`);
        }
  ...
  private generateSummaryReport(results: DiagnosticResult[]): void {
    ...
    if (foundItems.length > 0) {
      ...
      const validUrls = foundItems.filter(r => r.urlStatus === 'valid').length;
      const brokenUrls = foundItems.filter(r => r.urlStatus === 'broken').length;
      const missingUrls = foundItems.filter(r => r.urlStatus === 'missing').length;
      ...
      // Source breakdown
      const sourceBreakdown = foundItems.reduce((acc, item) => {
      ...
      // Lock status
      const lockedCount = foundItems.filter(r => r.posterLocked === true).length;
      const unlockedCount = foundItems.filter(r => r.posterLocked === false).length;
      const unknownLockCount = foundItems.filter(r => r.posterLocked === null).length;
      ...
      // Service breakdown
      const serviceBreakdown = foundItems.reduce((acc, item) => {
      ...
      // Problem items
      const problemItems = foundItems.filter(r => r.urlStatus === 'broken' || r.urlStatus === 'missing');
      
      if (problemItems.length > 0) {
        console.log('\n🚨 ITEMS NEEDING ATTENTION:');
        problemItems.forEach(item => {
          const statusEmoji = this.getUrlStatusEmoji(item.urlStatus);
          console.log(`   ${statusEmoji} ${item.title} (${item.service}) - ${item.urlStatus.toUpperCase()}`);
        });
      }
